...
public class EgovFileMngServiceImpl extends AbstractServiceImpl implements EgovFileMngService {
	...
	public String insertFileInfs(List fvoList, String memberseq) throws Exception {
		...
		if (fvoList.size() != 0) {
			atchFileId = fileMngDAO.insertFileInfs(fvoList, memberseq);
		}
		if(atchFileId == ""){
			atchFileId = null;
		}
	...
	public String updateMemberFileInfs(List fvoList, String memberseq) throws Exception {
		...
		if (fvoList.size() != 0) {
			atchFileId = fileMngDAO.updateMemberFileInfs(fvoList, memberseq);
		}
		if(atchFileId == ""){
			atchFileId = null;
		}
	...
	public String insertBoardFileInfs(List fvoList, String board_idx) throws Exception {
		...
		if (fvoList.size() != 0) {
			atchFileId = fileMngDAO.insertBoardFileInfs(fvoList, board_idx);
		}
		if(atchFileId == ""){
			atchFileId = null;
		}
	 ...
	 */
	 public String updateBoardFileInfs(List fvoList, String board_idx) throws Exception{
			...
			if (fvoList.size() != 0) {
				atchFileId = fileMngDAO.updateBoardFileInfs(fvoList, board_idx);
			}
			if(atchFileId == ""){
				atchFileId = null;
			}
	...
	public List<FileInfoVO> selectAllFileInf(Map<String, Object> param) throws Exception {
		return fileMngDAO.selectAllFileInf(param);
	}
